...
same convention as the `-data` siblings' `wnba_stats_NN_*_creation.py`.

| # | Module | Does | Invoked by |
|---|---|---|---|
| 01 | `wnba_stats_01_raw_json_scrape.py` | The sweep: discovery, season-level and per-game captures | `10_sweep.sh`, and `00_preflight.sh` via `--check` |
| 02 | `wnba_stats_02_leaguegamelog_player_topup.py` | PLAYER variant of `leaguegamelog`, landing additively beside the sweep's team rows | **no mode** — run manually |
| 03 | `wnba_stats_03_refill_empty.py` | Census + refill of payloads persisted as empty `{}` | `20_refill_empty.sh` |
| 99 | `wnba_stats_99_schedule_master_creation.py` | Schedule master + coverage index | `30_schedule_master.sh` |
...
| `backfill` | `1997:current` | `00,10,20,30,40,50` | 6 |
