# RUNBOOK — WNBA stats raw pipeline
...
| 50 | `50_publish_bundles.sh` | Refreshes the per-season tarballs on `wnba-stats-raw-json` | yes | no |
...
convention as the `-data` siblings' `wnba_stats_NN_*_creation.py`.

| # | Module | Scope | Reads | Invoked by |
|---|---|---|---|---|
| 01 | `wnba_stats_01_season_endpoints.py` | season-level payloads | — | `10_season_endpoints.sh` |
| 02 | `wnba_stats_02_game_endpoints.py` | one payload per game per endpoint | `leaguegamelog` on disk | `11_game_endpoints.sh` |
| 03 | `wnba_stats_03_period_boxscores.py` | per-period boxscores | `leaguegamelog` + each game's `playbyplayv3` on disk | `12_period_boxscores.sh` |
| 10 | `wnba_stats_10_leaguegamelog_player_topup.py` | PLAYER variant of `leaguegamelog` | — | **no mode** — run manually |
| 20 | `wnba_stats_20_refill_empty.py` | repair: empty `{}` payloads | the store | `20_refill_empty.sh` |
| 99 | `wnba_stats_99_schedule_master_creation.py` | schedule master + coverage index | the whole store | `30_schedule_master.sh` |
...
stage leaves a HOLE — the numbers must mean the same thing in the WWNBA twin.
...
`1997`, and `2026` means 2025-26. October rolls the current season
...
Droplet cron, **not** GitHub Actions: `stats.wnba.com` hangs rather than
...
`daily_refresh.sh` and `run_backfill.sh` are kept as thin shims that
