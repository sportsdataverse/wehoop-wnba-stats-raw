...
1. **League binding.** ``LEAGUE_ID = "00"/"10"`` literals (in
   ``period_capture.py`` / ``schedule_master.py`` / ``wnba_stats_01_raw_json_scrape.py``)
   and the ``NBA``/``WNBA`` config object ``wnba_stats_03_refill_empty.py`` imports must
...
2. **Transport wiring.** ``wnba_stats_01_raw_json_scrape.py``'s own module docstring states
   ...
   leftover pre-migration call site in ``wnba_stats_01_raw_json_scrape.py``'s per-period
   boxscore closure, and a second in ``wnba_stats_02_leaguegamelog_player_topup.py``
   masked by a bare ``except ImportError: return None`` around a cross-repo
   import (see 3). The first check used to scan only ``wnba_stats_01_raw_json_scrape.py``;
...
def test_wnba_stats_03_refill_empty_binds_the_right_league_config() -> None:
    """``wnba_stats_03_refill_empty.py`` imports NBA/WNBA by name and passes it to ``main()``
    -- the exact shape of the caught incident (importing a module that does
    not exist for this league)."""
    expected = _expected_league()
    source = (PYTHON / "wnba_stats_03_refill_empty.py").read_text(encoding="utf-8")
    imported = re.findall(
        r"from sportsdataverse\.scrape\.stats\.league_config import (\w+)", source
    )
    assert imported, "wnba_stats_03_refill_empty.py must import its LeagueConfig from league_config"
    assert imported == [expected.key.upper()], (
        f"wnba_stats_03_refill_empty.py imports {imported}, expected [{expected.key.upper()!r}]"
    )
    assert re.search(rf"main\({expected.key.upper()}\b", source), (
        f"wnba_stats_03_refill_empty.py must call main({expected.key.upper()}, ...)"
    )
...
def test_no_script_bypasses_the_session_transport() -> None:
    ...
    Scans every ``python/*.py``, not just ``wnba_stats_01_raw_json_scrape.py`` -- that
    narrower scope is exactly how a second bypass (in
    ``wnba_stats_02_leaguegamelog_player_topup.py``) survived the first version of this
    check. One closure called ``proxy_url=rr.next()`` after the rest of
    ``wnba_stats_01_raw_json_scrape.py`` had moved to ``transport=``; the other built its
