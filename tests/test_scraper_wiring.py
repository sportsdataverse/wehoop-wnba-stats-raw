"""Contract tests for the objects `wnba_stats_01_raw_json_scrape.main()` wires together.
...
SCRAPER = Path(__file__).resolve().parent.parent / "python" / "wnba_stats_01_raw_json_scrape.py"
...
def test_every_proxyhealth_attribute_the_scraper_uses_exists() -> None:
    ...
    assert not missing, f"wnba_stats_01_raw_json_scrape calls health.{missing} which ProxyHealth lacks"
...
def test_every_roundrobin_attribute_the_scraper_uses_exists() -> None:
    ...
    assert not missing, f"wnba_stats_01_raw_json_scrape calls rr.{missing} which RoundRobin lacks"
