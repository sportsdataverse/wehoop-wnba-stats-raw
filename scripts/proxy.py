"""Round-robin ProxyBonanza proxy pool for the shared stats.wnba.com request budget.

Ported verbatim from hoopR-nba-stats-data's ``nba_data_build/scrape/proxy.py``
(host-agnostic — same ProxyBonanza pool backs both stats.nba.com and
stats.wnba.com). Port of the R side's ``R/utils.R::get_proxy_ips`` /
``next_proxy``. Credentials (``PROXY_ENDPOINT`` / ``PROXY_KEY`` / ``PROXY_PKG``)
are read from the process environment at call time -- never hardcoded, never
logged in cleartext (use :func:`redact` before putting a proxy URL in a log
