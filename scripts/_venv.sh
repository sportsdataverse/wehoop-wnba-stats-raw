...
# SIBLING repo's venv (wehoop-wnba-stats-data/python/.venv) -- and because each
...
#
# Resolution order: $WNBA_VENV_PYTHON -> this repo's .venv (Windows, then POSIX).
# Callers that had their own override name keep it by exporting WNBA_VENV_PYTHON
# before sourcing.

_sdv_repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

if [ -n "${WNBA_VENV_PYTHON:-}" ]; then
...
if [ ! -x "$SDV_PY" ]; then
  ...
  echo "       run 'uv sync' in $_sdv_repo, or set WNBA_VENV_PYTHON" >&2
  exit 2
fi
