...
#
# Stage 10 of the WNBA stats raw pipeline. Every stage is independently
...
export SDV_PY_WNBA_STATS_TIMEOUT="${SDV_PY_WNBA_STATS_TIMEOUT:-90}"

echo "[$STAGE] $SEASONS (workers=$SCRAPE_WORKERS timeout=${SDV_PY_WNBA_STATS_TIMEOUT}s)"
# Resume is presence-on-disk, so Ctrl-C + rerun is always safe.
PYTHONUNBUFFERED=1 PYTHONIOENCODING=utf-8 "$PY" python/wnba_stats_01_season_endpoints.py "$SEASONS"
