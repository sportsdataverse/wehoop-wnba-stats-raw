...
#
# Stage 20 of the WNBA stats raw pipeline. Every stage is independently
...
# so `git checkout -- wnba_stats/` undoes a bad run.
echo "[$STAGE] empty-payload census + refill for $SEASONS"
"$PY" python/wnba_stats_03_refill_empty.py --check "$SEASONS" || true
if [ "${REFILL_APPLY:-1}" = "1" ]; then
  "$PY" python/wnba_stats_03_refill_empty.py "$SEASONS"
else
