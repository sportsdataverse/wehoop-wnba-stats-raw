...
#   tail -f logs/wnba_stats_20_refill_empty.log
#
set -uo pipefail
...
LOG="$REPO/logs/wnba_stats_20_refill_empty.log"
mkdir -p "$REPO/logs"
...
if [ "${1:-}" = "--check" ]; then
  "$SDV_PY" python/wnba_stats_20_refill_empty.py --check
  exit $?
fi
...
"$SDV_PY" python/wnba_stats_20_refill_empty.py "$@" 2>&1 | tee -a "$LOG"
status=${PIPESTATUS[0]}
