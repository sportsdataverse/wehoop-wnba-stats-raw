"""Schedule master + coverage index for the stats.wnba.com raw archive (spec §5).
...
per-game stat calls made the WBB step crawl on a tree this size.

Season and season_type are derived from each row's ``SEASON_ID`` (leading
digit = type, remainder = calendar year), never from the season directory
label — directory labels have historically mixed conventions between endpoint
families, and rows are the only truth. WNBA seasons are BARE CALENDAR YEARS
("2023"), not the NBA span form ("2023-24"): the season fits inside one year.

Game ids are pinned ``Utf8`` at the boundary: WNBA ids carry the "10" league
prefix ("1022300001") and must never round-trip through a float cast.
"""
...
GAME_FILE_RE = re.compile(r"^(10\d{8})\.json$")

#: SEASON_ID leading digit -> season_type label. The WNBA log ships 2 and 4;
#: 1/3/5 are kept for the same digit namespace the API uses league-wide.
SEASON_TYPES = {
    "1": "preseason",
    "2": "regular_season",
    "3": "all_star",
...
def _season_label(year: int) -> str:
    """2023 -> "2023" — WNBA seasons are single calendar years, never spans."""
    return str(year)
            ...
            for result_set in payload.get("resultSets") or []:
                ...
                for row in result_set.get("rowSet") or []:
                    ...
                    rec = games.setdefault(
                        ...
                        {
                            "game_id": gid,
                            "season": _season_label(int(sid[1:])),
