...
def is_contentless(payload: Any) -> bool:
    ...
    stats.wnba.com never answers with an empty body: an endpoint with no rows
    ...
    complete, so a backfill no-ops. 3,872 files in this repo (3.55%, five times
    the NBA sibling's rate) reached that state before this guard.
...
def _result_tables(payload: Any) -> list[dict]:
    """Every ``{name, headers, rowSet}`` table in a stats.wnba.com payload.
    ...
    * ``resultSet`` SINGULAR, holding one table as a dict (leagueleaders,
    ...
    no result tables at all. The five families are declared in sdv-py's
    ``sportsdataverse/schemas/raw/nba_stats_*.yaml``.
    """
