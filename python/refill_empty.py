...
season complete. A backfill no-ops. wehoop-wnba-stats-raw reached 3,347 such files
...
* The deleted files are tracked in git, so `git checkout -- wnba_stats/` restores
...
def main(argv: list[str] | None = None) -> int:
    ...
    import sportsdataverse.nba.wnba_stats as stats

    timeout = os.environ.get("SDV_PY_NBA_STATS_TIMEOUT", "30")
    pool = load_proxies()
    if not pool and not args.allow_direct:
        # Same contract as the main sweep: un-proxied stats.wnba.com calls HANG
        ...
        return 1
    rr = RoundRobin(pool) if pool else None
    _log(
        f"proxy pool: {len(pool)} entries | timeout={timeout}s"
        if pool
        else f"no proxy pool; --allow-direct given | timeout={timeout}s"
    )

    def fetch(endpoint: str, kwargs: dict) -> object:
        # This repo passes a per-request proxy_url rather than a transport
        # object -- it is on an older scraper generation than the NBA sibling.
